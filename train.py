...
class SRDataset(Dataset):
    def __init__(self, hr_dir, scale=2, patch_size=128, in_ch=3, train=True):
        super().__init__()
        self.paths = sorted([p for p in glob.glob(str(Path(hr_dir)/"*")) if p.lower().endswith((".png",".jpg",".jpeg",".bmp",".tiff",".webp"))])
        if not self.paths:
...
def prepare_decoder_qat(decoder: nn.Module, example_inputs, backend="fbgemm"):
    ...
    dec_qat = prepare_qat_fx(decoder, qconfig_dict, example_inputs=example_inputs)
    return dec_qat
...
def main():
    ...
    train_dl = DataLoader(train_ds, batch_size=args.bs, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print(f"[{epoch}/{args.epochs}] train={train_loss:.4f}  val={val_loss:.4f}  ~comm={avg_kb:.1f} KB/img")
        ...
        torch.save({
            ...
            "args": vars(args)
        }, os.path.join(args.out_dir, f"ckpt_epoch{epoch}.pth"))
    ...
    # convert decoder to INT8
    print("Converting decoder to INT8 ...")
    ...
    print("Saved final_int8_decoder.pth")
